import type { ConferenceProvider } from "../integrations/types";
...
/** Simple Icons path data for each provider's mark (viewBox 0 0 24 24). */
const BRAND_PATH: Record<Exclude<ConferenceProvider, "other">, string> = {
...
/** A neutral video-camera outline for providers we do not recognize. */
function genericCamera(): SVGElement {
  const svg = svgEl("svg", {
    viewBox: "0 0 24 24",
    width: "14",
    height: "14",
...
/** The Join button's leading glyph for a provider; inherits the button color. */
export function conferenceIcon(provider: ConferenceProvider): SVGElement {
  if (provider === "other") return genericCamera();
  const svg = svgEl("svg", {
    viewBox: "0 0 24 24",
    width: "14",
    height: "14",
  ...
  svg.appendChild(svgEl("path", { d: BRAND_PATH[provider] }));
  return svg;
}
