...
/**
 * The official multicolor Google Calendar logo as inline SVG: a white body with
 * a colored frame and the blue "31". The body is transparent, so render it on a
 * white ground. Paths are Google's, kept in their original paint order.
 */
const PATHS: ReadonlyArray<{ d: string; fill: string }> = [
  ...
  { d: "m390.737 512 121.263-121.263h-121.263z", fill: "#ff4131" },
];

export function googleCalendarLogo(size: number): SVGElement {
  const svg = svgEl("svg", {
    viewBox: "0 0 512 512",
    width: String(size),
    height: String(size),
  ...
  for (const path of PATHS) svg.appendChild(svgEl("path", { d: path.d, fill: path.fill }));
  return svg;
}
